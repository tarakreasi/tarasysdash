...
### 2. Compliance & Security (RLS)
Every table containing a `tenant_id` **MUST** have a corresponding RLS Policy.
- **Why**: "Defense in Depth".
- **Action**: Use `DB::statement("CREATE POLICY ...")` in the `up()` method.
- **Reference**: See `standards/global/compliance.md`.
