...
This guide details how to install the `tara-agent` on Windows Server 2019/2022 using PowerShell and the built-in Service Control Manager (`sc.exe`).

## Prerequisites
- **Administrator Privileges** are required.
- The `agent-windows-amd64.exe` binary.

## Installation Script (`install.ps1`)

Save the following as `install.ps1` in the same folder as your `agent-windows-amd64.exe`:
...
## Manual Verification
1. Open `services.msc`.
2. Look for **Tara Infrastructure Agent**.
